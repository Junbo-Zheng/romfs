...
class RomFSReader:
    ...
    def unpack(self, outdir: str | Path) -> None:
    ...
    def find(self, path: str) -> RomFSNode | None:
        """Locate a node by slash-separated path relative to root.

        Drills down the children by path segment instead of scanning the whole
        tree, so it stays cheap for deep images. ``""`` or ``"/"`` returns the
        root. Returns ``None`` if any segment does not match.
        """
        node = self.root
    ...
    def read_path(self, path: str) -> bytes:
        """Return the payload of the entry at ``path`` (regular file or symlink)."""
        node = self.find(path)
        if node is None:
            raise FileNotFoundError(f"path not found in image: {path!r}")
        if node.type is EntryType.DIRECTORY:
            raise IsADirectoryError(f"path is a directory: {path!r}")
        return self.read(node)

    def listdir(self, path: str = "") -> list[RomFSNode]:
        """Return the direct children of the directory at ``path`` (root by default)."""
        node = self.find(path)
        if node is None:
            raise FileNotFoundError(f"path not found in image: {path!r}")
        if node.type is not EntryType.DIRECTORY:
    ...
    def _parse_node(self, offset: int) -> RomFSNode:
        ...
        executable = bool(next_off & ROMFH_EXEC)
        ...
        if etype is EntryType.HARDLINK:
            ...
            # read()/unpack() see the linked content; directory targets are
    ...
    def _resolve_hardlink(self, node: RomFSNode, target_off: int) -> None:
        ...
        node.executable = bool(t_next & ROMFH_EXEC)
