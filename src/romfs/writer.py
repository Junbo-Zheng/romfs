...
class RomFSWriter:
    ...
    def _make_node(cls, path: Path) -> RomFSNode | None:
        ...
        if path.is_file():
            ...
            return RomFSNode(
                ...
                size=st.st_size,
                src_path=str(path),
                # romfs stores only the executable bit, not full Unix mode —
                # preserve it when the source file is executable by anyone,
                # matching genromfs (which sets ROMFH_EXEC on any X bit).
    ...
    def _render(self, node: RomFSNode, buf: bytearray) -> None:
        ...
        # Only regular files carry the executable bit; dirs/symlinks don't.
        exec_bits = (
            ROMFH_EXEC if (node.type is EntryType.REGULAR and node.executable) else 0
        )
        next_field = (next_off | type_bits | exec_bits) & 0xFFFFFFFF
        ...
        if node.type is EntryType.REGULAR:
            assert node.src_path is not None, (
